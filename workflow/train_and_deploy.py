...
def train_and_register_model(
    ...
    logger.info(
        f"training data and register model for {ticker} from {start_date} to {end_date}"
    )
    for model in models:
        train_and_log_model(ticker, start_date, end_date, model)
    ...
    best_run = client.search_runs(
        ...
        max_results=1,
        order_by=["metrics.cumulative_return DESC"],
    )[0]
...
def transition_model_stage():
    ...
    logger.info(
        f"transition model {REGISTER_MODEL_NAME} version {version} to production"
    )

    client.set_registered_model_alias(
        name=REGISTER_MODEL_NAME, alias="prd", version=version
    )
