    ...
    def step(self, action):
        ...
        if action == 0:  # Sell
            if self.shares_owned > 0:
                self.balance += (
                    self.data["price_next_day"][self.current_step]
                    * self.shares_owned
                )
                self.shares_owned = 0
            ...
            if self.balance >= self.data["price_next_day"][self.current_step]:
                share_buy = math.floor(
                    self.balance
                    / self.data["price_next_day"][self.current_step]
                )
                self.shares_owned += share_buy
                self.balance -= (
                    self.data["price_next_day"][self.current_step] * share_buy
                )
        ...
        self.net_worth = self.balance + (
            self.shares_owned * self.data["Close"][self.current_step]
        )
    ...
    def _discretize_state(self, state):
        ...
        for i in range(len(state)):
            if state_bounds[i][0] == state_bounds[i][1]:
                ...
                scaled_value = (state[i] - state_bounds[i][0]) / (
                    state_bounds[i][1] - state_bounds[i][0]
                )
                discrete_value = int(
                    np.clip(
                        scaled_value * (self.q_table.shape[i] - 1),
                        0,
                        self.q_table.shape[i] - 1,
                    )
                )
